...
class SmallEvent(UI):
    ...
    def run(self):
        if datetime.now() < self.config.Smallevent_deadline:
            ORC_API = self.ocr_api_init()
            if ORC_API:
                #第一栏
                if self.config.Smallevent_lane_one_entry:
                ...
                #第二栏
                if self.config.Smallevent_lane_two_entry:
                    page_area = (0, 0, 1280, 720)
                    exclude_text = [t.strip() for t in self.config.Smallevent_lane_two_exclude_text.split(',')]
                    goPage_result = self.goto_sevenD_page_general(
                        page_area, ORC_API,
                        button_text=self.config.Smallevent_lane_two_button_text,
                        exclude_text=exclude_text,
                        entry_index=2,
                        page_text=self.config.Smallevent_lane_two_page_text)
                    self.sevenD_harvest(page_area, ORC_API, goPage_result)
                #第三栏
